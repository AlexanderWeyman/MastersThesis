...
set shield [expr 0.65*$sigma]
#set shield [expr 0.5*$sigma]
#set shield [expr 0.9*$sigma]
...
#set equi_dt 0.01
set equi_steps_per_iteration 100
...
set distr_r_max $box_l
#adjusted to box_l/2
#set distr_r_max [expr $box_l/2.0] ##################################### CHANGE AFTER SINGLE CHAIN SIMULATIONS #############################################################################
...
if { [file exists "$output_checkpointing_path$output_checkpointing_file"] } {
    ...
    puts $analyze_file_equi "\#time\tr_e\terr_r_e\tr_g\terr_r_g\tr_h\terr_r_h\tE_tot\tE_kin\tE_coul"
...
proc doOnExit {} {
    ...
    global list_simulation_time list_end_to_end_distance list_end_to_end_distance_error list_radius_of_gyration list_radius_of_gyration_error list_hydrodynamic_radius list_hydrodynamic_radius_error list_energy_total list_energy_kinetic list_energy_coulomb analyze_file_equi vtf_file_equi output_checkpointing_path output_checkpointing_file

    #update analyze file
    foreach t $list_simulation_time re $list_end_to_end_distance err_re $list_end_to_end_distance_error rg $list_radius_of_gyration err_rg $list_radius_of_gyration_error rh $list_hydrodynamic_radius err_rh $list_hydrodynamic_radius_error etot $list_energy_total ekin $list_energy_kinetic ecoul $list_energy_coulomb {
        puts $analyze_file_equi "$t\t$re\t$err_re\t$rg\t$err_rg\t$rh\t$err_rh\t$etot\t$ekin\t$ecoul"
    }
    flush $analyze_file_equi
    ...
    if { [info exists save_all_checkpoints] && $save_all_checkpoints } {
        ...
        file copy "$output_checkpointing_path$output_checkpointing_file" "$output_sub_checkpointing_path${simulation_identifier}_[format "%.4f" [setmd time]]_checkpointing.txt"
    }
...
#for {set i 0} { $i <= $equi_iterations } { incr i} {
...
while { [setmd time] < $simulation_max_time } {
