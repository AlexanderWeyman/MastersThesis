...
# Write velocities of the system particles to the file. Similar to writevcf.
# (VTF Velocity Format)
proc writevvf { file args } {
    # set defaults
    set short 0
    set pids "all"
    set userdata 0
	...
	for { set pid 0 } { $pid <= $max_pid } { incr pid } {
	    if {[part $pid] != "na"} then {
                puts -nonewline $file [part $pid print v]
		if { [llength $userdata] } then {
		    puts $file [ lindex $userdata $pid ]
		} else {
		    puts $file ""
	...
	foreach pid $pids {
	    if {[part $pid] != "na"} then {
                puts -nonewline $file "[vtfpid $pid] [part $pid print v]"
		if { [llength $userdata] } then {
		    puts $file [ lindex $userdata $pid ]
		} else {
		    puts $file ""
