#!/usr/bin/python
"""This script calculates the velocity distribution of particle types along the z axis from vtf files."""
...
for current_frame in xrange(n_frames):
    ...
    for t in list_ptypes:
        ...
        for i in xrange(len(current_vels)):
            ...
            vel_distributions[t][bin_idx] += current_vels[i]
...
for t in list_ptypes:
    for bin_idx, counter in enumerate(avg_counters[t]):
        if counter > 0.0:
            vel_distributions[t][bin_idx] /= counter


#file to save data
split_filename = os.path.splitext(filename_velocities)
output_filename = "{0}{1}_Velocity_Distribution_{2}bins.txt".format(output_path, os.path.basename(split_filename[0]), n_bins)
...
for t in list_ptypes:
    output_file.write("\tv_x_type{0}".format(int(t)))
output_file.write("\n")

for i in xrange(n_bins):
    output_file.write("{}".format(bin_edges[i]))
    for t in list_ptypes:
        output_file.write("\t{}".format(vel_distributions[t][i]))
    output_file.write("\n")
