...
try:
    ...
    l_b = float(sys.argv[4])
    q_min = float(sys.argv[5])
    q_max = float(sys.argv[6])
    q_step = float(sys.argv[7])
except:
...
try:
    argSympy = int(sys.argv[8])
    useSympy = bool(argSympy)
except:
    useSympy = False
...
#q_max = 20
#q_step = 0.1
plotPath = "matrix_plots/eta{0}_sclen{1}/".format(eta, delta_s)
if plotMatrices and not os.path.isdir(plotPath):
...
def calc_s_matrix(q):
    ...
    if useSympy:
        matr = (np.identity((delta_s+1)*N+1) - calc_c_matrix(q)).tolist()
        inv_matr = sympy.Matrix(matr)**(-1)
        return np.array(inv_matr.tolist())
...
#specify q values to calculate partial structure factors from
q_vals = np.arange(q_min, q_max, q_step)
q_vals = np.append(q_vals, q_max) #q max should be the last datapoint in q_vals
len_q_vals = len(q_vals)
...
if useSympy:
    output_filename = "{0}partial_structurefactors_N{1}_sclen{2}_eta{3}_lB{4}_q{5}-{6}-{7}_zeta{8}_sympy.txt".format(output_path, N, delta_s, eta, l_b, q_min, q_max, q_step, zeta)
else:
    output_filename = "{0}partial_structurefactors_N{1}_sclen{2}_eta{3}_lB{4}_q{5}-{6}-{7}_zeta{8}.txt".format(output_path, N, delta_s, eta, l_b, q_min, q_max, q_step, zeta)
...
for current_q_idx, current_q in enumerate(q_vals):
    print "Iteration {0}/{1}: Calculate partial structure factors for q={2}".format(current_q_idx+1, len_q_vals, current_q)
    
    s_matrix = calc_s_matrix(current_q)
    
    if plotMatrices:
        fig = plt.figure()
        ax = fig.add_subplot(111)
        cax = ax.matshow(s_matrix)
        fig.colorbar(cax)
        plt.title("q = {}".format(current_q))
        
        plt.savefig("{0}s_matrix_N{1}_sclen{2}_eta{3}_lB{4}_q{5:.3f}.png".format(plotPath, N, delta_s, eta, l_b, current_q))
        plt.close()
    ...
    output_file.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(current_q, S_BB, S_TT, S_BT, S_TC, S_CC, S_QQ, S_NN))
    output_file.flush()
...
if plotMatrices:
    print "All matrix plots were saved in '{}'.".format(plotPath)
